...
= Topic
The main topic of this lab is to manipulate the sinewave parameters, namely: amplitude, frequency and phase, in *Makie*, which is an interactive graphics library that allows for the creation and manipulation of high-quality visualizations. 

#exo[Case of Sinewave][Consider the code shown hereafter, in which we cant to control and dynamically update the _amplitude_, _frequency_ and _phase_ of a sinewave. Those settings are represented by reactive variables, called *#text(smallcaps("Observables"))*. The resulting interactive graph is shown in @fig:makie-sinewave. By changing each slider, we automatically update and trigger some actions _(e.g., y-limits)_ in response to changes in its value.]
